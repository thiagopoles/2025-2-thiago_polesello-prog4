...
function chamarBackend() {
    ...
    let url = `http://localhost/2025-2-thiago_polesollo-prog4/PHP/consultasPTQA/pressaoAtmosferica.php?dataInicial=${valorDataInicial}&dataFinal=${valorDataFinal}`;
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            if (data.length > 0) {
                ...
                const mediaCo2 = data.map(item => item.pressao);
