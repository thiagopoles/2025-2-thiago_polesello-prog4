...
function chamarBackend(event) {
    ...
    let url = `http://localhost/2025-2-thiago_polesello-prog4/php/consultaMabel/temperaturaNinho.php?dataInicial=${valorDataInicial}&dataFinal=${valorDataFinal}` 
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            if (data.length > 0) {
                ...
                const valores = data.map(item => item.temp_ninho);


                const ctx = document.getElementById('temperaturaNinho').getContext('2d');
